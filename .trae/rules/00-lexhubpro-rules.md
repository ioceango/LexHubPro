...
> 本文件是 **Trae** 在本仓库的规则入口（`.trae/rules/`）。
>
...
细则展开见 `docs/rules/0X-*.md`。技能包权威目录：`.agent/skills/`。MCP 通用清单：仓库根 `.mcp.json`。
...
规约变更只改 `.agent/`；本文件为纯路标，禁止在其中新增、改写或扩写规则条款。禁止把 `.agent/rules.md` 再复制进 `.trae/rules/`。
